<template>
  ...
  <h1 style="font-size: 28px; margin-bottom: 20px">产品收购</h1>
  <Dialogue />
  <span style="height: 32px">
    ...
    <a-date-picker
      ...
      @panelChange="selectHanlder"
      placeholder="按年份查询订单车"
    />
        ...
        <template v-else-if="column.dataIndex === 'operation'">
          <div class="editable-row-operations">
            <span v-if="editableData[record.orderId]">
              <a-popconfirm
                title="确认修改?"
                @confirm="update(record.orderId)"
                cancel-text="取消"
                ok-text="保存"
              >
                <a>保存</a>
              </a-popconfirm>
              <a-typography-link @click="cancel(record.orderId)"
                >取消</a-typography-link
              >
            </span>
            <span v-else>
...
const edit = (orderId: string) => {
  editableData[orderId] = cloneDeep(
    dataSource.value.filter((item) => orderId === item.orderId)[0]
  );
};
...
const update = async (orderId: string) => {
  if (
    editableData[orderId]["year"] > 2100 ||
    editableData[orderId]["year"] < 1900
  )
    return message.warn("请输入正确的年份");
  try {
  ...
  Modal.confirm({
    ...
    async onOk() {
      try {
        ...
        let res = await deleteOrderCarBatch({
          orderIds: state.selectedRowKeys,
        });
        if (res.data.code === 200) {
