<template>
  ...
  <h1 style="font-size: 28px;margin-bottom: 20px">批发中心产品销售表</h1>
  <Dialogue/>
...
import { getProductOuttock } from "@/request/api";
  ...
  dataSource.value =  dataSourceCopy.value.filter(item => {
    return (
            ...
            ( 
              ...
              item.gknumber.toLowerCase().includes(keywords) ||
              item.total.toLowerCase().includes(keywords) ||
