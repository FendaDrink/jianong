<template>
  ...
  <h1 style="font-size: 28px;margin-bottom: 20px">生产计划管理</h1>
  <Dialogue/>
...
import {getProductionPlan, updateProductionPlan, deleteProductionPlan} from "@/request/enterprise";
...
const deleteItem = async (key:string) => {
  let res = await deleteProductionPlan(key);
...
const update= async (key:string) => {
  ...
  try{
    let res = await updateProductionPlan(editableData[key]);
    if(res.data.code === 200){
